...
#define TEMPERATURE_SAMPLE_BUFFER (0)
#define HUMIDITY_SAMPLE_BUFFER    (1)
...
#define EEPROM_BACKUP_TEMPERATURE_OFFLINE_OFFSET (EEPROM_BACKUP_PM10P0_CAL_OFFSET + 4)
...
void updateDisplayedSensors() {
    ...
    const uint8_t NUM_DISPLAY_PAGES = 1;
    ...
    if(first || ((current_millis - previous_display_update) >= display_update_interval)) {
        ...
        default:
            ...
            updateLCD("BP ", 0, 1, 3, false);
            if(init_bmp280_ok) {
                updateLCD("BP ", 0, 1, 3, false);
                if(pressure_ready || (sample_buffer_idx > 0)) {
                    updateLCD(pressure_pa / 1000.0f, 3, 1, 5, false);
                }
                else {
                    updateLCD("----", 3, 1, 5, false);
                }
            }
            break;
        }
...
void initializeHardware(void) {
    ...
    Serial.println(F(" |          Base Sensor Suite         |"));
...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        eeprom_write_block("aqev2_base_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
boolean publishHeartbeat() {
    ...
    snprintf(scratch, SCRATCH_BUFFER_SIZE-1,
             ...
             "\"converted-value\":%d,"
             "\"firmware-version\":\"%s\","
             "\"publishes\":[\"temperature\",\"humidity\"%s],"
             "\"counter\":%lu"
